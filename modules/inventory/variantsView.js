...
export function openVariantDetailModal(variant, refreshCallback) {
  ...
  const isCtl = Boolean(variant.isCutToLength || variant.rollLength || prod?.cut_to_length);
  const whCtl = isCtl ? cutToLengthService.getVariantStock('wh-1', variant.id) : null;
  const offCtl = isCtl ? cutToLengthService.getVariantStock('wh-2', variant.id) : null;
  const ctlUnit = variant.rollUnit || variant.unit || prod?.base_unit || unit;
  ...
  const contentHtml = `
    <div class="space-y-6 text-xs">
      ...
      <section class="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-xs space-y-4">
        ...
        ${isCtl ? `
          <div class="p-3.5 bg-blue-50/60 rounded-xl border border-blue-200/80 flex items-center justify-between">
            <div class="flex items-center gap-2">
              <span class="text-base">📏</span>
              <div>
                <span class="font-bold text-blue-900 text-xs block">Roll / Cut-to-Length Packaging</span>
                <span class="text-[11px] text-slate-600 font-medium">Standard Roll Length: <strong>${Number(variant.rollLength || variant.rollSize || 5000).toLocaleString()} ${ctlUnit}</strong></span>
              </div>
            </div>
            <span class="px-2.5 py-1 bg-white border border-blue-200 text-blue-700 font-bold text-[10px] rounded-lg">
              Variant Cut Enabled
            </span>
          </div>
        ` : ''}
      ...
      <section class="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-xs space-y-4">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div class="p-4 rounded-xl bg-blue-50/50 border border-blue-200/70 space-y-2">
            <div class="flex items-center justify-between">
              <span class="text-[10px] font-bold text-blue-700 uppercase tracking-wider block">Main Warehouse (wh-1)</span>
              ${isCtl && whCtl ? `<span class="px-2 py-0.5 rounded bg-blue-100 text-blue-800 text-[10px] font-bold">${whCtl.fullRolls} Rolls</span>` : ''}
            </div>
            ...
            ${isCtl && whCtl ? `
              <div class="text-[11px] text-slate-600 bg-white p-2.5 rounded-lg border border-blue-100 space-y-0.5">
                <div>📦 Full Rolls: <strong class="text-blue-900">${whCtl.fullRolls}</strong> (${(whCtl.fullRolls * whCtl.rollLength).toLocaleString()} ${ctlUnit})</div>
                <div>✂️ Loose Pieces: <strong class="text-amber-800">${whCtl.loosePieces.length} pc(s)</strong> (${whCtl.loosePieces.reduce((s, p) => s + p, 0).toLocaleString()} ${ctlUnit})</div>
                ${whCtl.loosePieces.length > 0 ? `<div class="text-[10px] text-slate-400 font-mono">[${whCtl.loosePieces.map(n => n.toLocaleString()).join(', ')}]</div>` : ''}
              </div>
            ` : ''}
          </div>

          <div class="p-4 rounded-xl bg-amber-50/50 border border-amber-200/70 space-y-2">
            <div class="flex items-center justify-between">
              <span class="text-[10px] font-bold text-amber-800 uppercase tracking-wider block">Office Hub (wh-2)</span>
              ${isCtl && offCtl ? `<span class="px-2 py-0.5 rounded bg-amber-100 text-amber-900 text-[10px] font-bold">${offCtl.fullRolls} Rolls</span>` : ''}
            </div>
            ...
            ${isCtl && offCtl ? `
              <div class="text-[11px] text-slate-600 bg-white p-2.5 rounded-lg border border-amber-100 space-y-0.5">
                <div>📦 Full Rolls: <strong class="text-blue-900">${offCtl.fullRolls}</strong> (${(offCtl.fullRolls * offCtl.rollLength).toLocaleString()} ${ctlUnit})</div>
                <div>✂️ Loose Pieces: <strong class="text-amber-800">${offCtl.loosePieces.length} pc(s)</strong> (${offCtl.loosePieces.reduce((s, p) => s + p, 0).toLocaleString()} ${ctlUnit})</div>
                ${offCtl.loosePieces.length > 0 ? `<div class="text-[10px] text-slate-400 font-mono">[${offCtl.loosePieces.map(n => n.toLocaleString()).join(', ')}]</div>` : ''}
              </div>
            ` : ''}
          </div>
        </div>
...
function openVariantModal(variant = null, onSaved) {
    ...
    <form id="variant-form" class="space-y-5 text-xs">
      ...
      <section class="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-xs space-y-4">
        <div class="flex items-center justify-between border-b border-slate-100 pb-3">
          <h3 class="text-xs font-bold uppercase tracking-wider text-slate-700 flex items-center gap-2">
            <span>📏</span>
            <span>3. Roll / Cut-to-Length Packaging</span>
          </h3>
          <span class="text-[10px] font-medium text-slate-400">Configure roll length and measurement unit</span>
        </div>

        <div class="flex items-center gap-2">
          <input type="checkbox" id="var-is-ctl" ${variant && (variant.isCutToLength || variant.rollLength) ? 'checked' : ''} class="rounded border-slate-300 text-[#138FCB] focus:ring-0">
          <label for="var-is-ctl" class="font-semibold text-slate-700 cursor-pointer">Enable Roll &amp; Cut-to-Length for this Variant</label>
        </div>

        <div id="var-ctl-config-box" class="${variant && (variant.isCutToLength || variant.rollLength) ? '' : 'hidden'} grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
          <div class="space-y-1.5">
            <label class="text-xs font-semibold text-slate-700" for="var-roll-length">Standard Roll Length <span class="text-red-500">*</span></label>
            <input type="number" id="var-roll-length" min="1" step="any" value="${variant?.rollLength || variant?.rollSize || 5000}" placeholder="5000" class="w-full text-xs font-bold rounded-xl border border-slate-200 focus:border-[#138FCB] py-2.5 px-3 text-slate-800 bg-white shadow-2xs">
            <span class="text-[10px] text-slate-400">e.g. 5,000 ft wire coil, 450 ft auger, 400 ft auger</span>
          </div>

          <div class="space-y-1.5">
            <label class="text-xs font-semibold text-slate-700" for="var-roll-unit">Measurement Unit</label>
            <input type="text" id="var-roll-unit" value="${variant?.rollUnit || 'ft'}" placeholder="ft" class="w-full text-xs font-bold rounded-xl border border-slate-200 focus:border-[#138FCB] py-2.5 px-3 text-slate-800 bg-white shadow-2xs">
          </div>
        </div>
      </section>
    </form>
  ...
  openModal({
    ...
    onOpen: (modalEl) => {
      ...
      const ctlCheckbox = modalEl.querySelector('#var-is-ctl');
      const ctlBox = modalEl.querySelector('#var-ctl-config-box');
      ...
      modalEl.querySelector('#variant-form').onsubmit = (e) => {
        ...
        if (isCutToLength && targetVarId) {
          const curStock = cutToLengthService.getVariantStock('wh-1', targetVarId);
          cutToLengthService.saveVariantStock('wh-1', targetVarId, {
            fullRolls: curStock?.fullRolls ?? 5,
            rollLength,
            loosePieces: curStock?.loosePieces ?? [],
            unit: rollUnit || 'ft'
          });
        }
