...
function openCreateGatepassModal(onSaved, gatepassToEdit = null) {
  ...
  const renderRowHtml = (variantId = null, whQty = '', offQty = '') => {
    ...
    return `
      <tr class="gp-line-row hover:bg-slate-50/70 transition-colors">
        <td class="p-3">
          <select class="gp-item-var w-full text-xs font-semibold rounded border border-slate-300 focus:border-[#138FCB] focus:ring focus:ring-blue-100 py-1.5 px-2 bg-white shadow-2xs">
            ${variants.map(v => `<option value="${v.id}" data-unit="${v.unit || 'PCS'}" ${v.id === vId ? 'selected' : ''}>${v.name} (${v.sku})</option>`).join('')}
          </select>
        </td>
        <td class="p-3 text-center">
          <span class="wh-stock-indicator block text-[10px] text-slate-500 font-semibold mb-1">
            WH Stock: ${whStock.toLocaleString()} ${unit}
          </span>
          <input type="number" min="0" value="${wVal}" placeholder="" class="gp-wh-qty w-24 text-center text-xs font-extrabold rounded border border-blue-300 focus:border-blue-500 focus:ring-1 py-1.5 px-2 bg-white text-blue-900 shadow-2xs">
        </td>
        <td class="p-3 text-center">
          <span class="office-stock-indicator block text-[10px] text-slate-500 font-semibold mb-1">
            Office Stock: ${officeStock.toLocaleString()} ${unit}
          </span>
          <input type="number" min="0" value="${oVal}" placeholder="" class="gp-office-qty w-24 text-center text-xs font-extrabold rounded border border-amber-300 focus:border-amber-500 focus:ring-1 py-1.5 px-2 bg-white text-amber-900 shadow-2xs">
        </td>
        <td class="p-3 text-right">
          <span class="gp-total-calc font-black text-slate-900 text-sm">${lineTotal > 0 ? `${lineTotal.toLocaleString()} ${unit}` : '—'}</span>
        </td>
        <td class="p-3 text-center">
          <button type="button" class="gp-remove-row-btn w-7 h-7 inline-flex items-center justify-center rounded-lg text-slate-400 hover:text-rose-600 hover:bg-rose-50 transition-colors cursor-pointer" title="Remove product line">
  ...
  const contentHtml = `
    <form id="create-gp-form" class="space-y-6 text-xs">
      ...
      <section class="bg-white p-5 rounded-xl border border-slate-200/80 shadow-xs space-y-4" data-purpose="client-and-logistics">
        <div class="flex items-center justify-between border-b border-slate-100 pb-3">
          <h3 class="text-xs font-bold uppercase tracking-wider text-slate-500 flex items-center space-x-2">
            ...
            <span>Client &amp; Logistics Details</span>
          </h3>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
          ...
          <div class="md:col-span-12 space-y-1.5">
            ...
            <input type="text" id="gp-customer-name" required placeholder="Enter Customer / Farm Name" value="${isEdit ? (gatepassToEdit.customerName || '') : ''}" class="w-full text-xs font-medium rounded-lg border border-slate-300 focus:border-[#138FCB] focus:ring focus:ring-blue-100 py-2.5 px-3 text-slate-800 bg-white shadow-2xs">
          </div>

          <!-- Vehicle Number -->
          <div class="md:col-span-4 space-y-1.5">
            <label class="text-xs font-semibold text-slate-700" for="gp-vehicle">Vehicle Number <span class="text-red-500">*</span></label>
            <input type="text" id="gp-vehicle" required placeholder="e.g. LES-9412 Truck" value="${isEdit ? (gatepassToEdit.vehicleNumber || '') : ''}" class="w-full text-xs font-medium rounded-lg border border-slate-300 focus:border-[#138FCB] focus:ring focus:ring-blue-100 py-2.5 px-3 text-slate-800 bg-white shadow-2xs">
          </div>

          <!-- Driver Name -->
          <div class="md:col-span-4 space-y-1.5">
            <label class="text-xs font-semibold text-slate-700" for="gp-driver">Driver Name <span class="text-red-500">*</span></label>
            <input type="text" id="gp-driver" required placeholder="e.g. Muhammad Rasheed" value="${isEdit ? (gatepassToEdit.driverName || '') : ''}" class="w-full text-xs font-medium rounded-lg border border-slate-300 focus:border-[#138FCB] focus:ring focus:ring-blue-100 py-2.5 px-3 text-slate-800 bg-white shadow-2xs">
          </div>

          <!-- Driver Phone -->
          <div class="md:col-span-4 space-y-1.5">
            <label class="text-xs font-semibold text-slate-700" for="gp-driver-phone">Driver Phone</label>
            <input type="tel" id="gp-driver-phone" placeholder="e.g. +92 345 6789012" value="${isEdit ? (gatepassToEdit.driverPhone || '') : ''}" class="w-full text-xs font-medium rounded-lg border border-slate-300 focus:border-[#138FCB] focus:ring focus:ring-blue-100 py-2.5 px-3 text-slate-800 bg-white shadow-2xs">
          </div>
        </div>
      </section>

      <!-- SECTION 2: Items & Multi-Location Stock Allocation Table -->
      <section class="bg-white p-5 rounded-xl border border-slate-200/80 shadow-xs space-y-4" data-purpose="line-items-section">
        <div class="flex items-center justify-between">
          <div class="flex items-center space-x-2">
            <h3 class="text-xs font-bold uppercase tracking-wider text-slate-500">Items &amp; Dual-Location Allocation</h3>
          ...
          <button type="button" id="add-gp-row-btn-top" class="inline-flex items-center space-x-1.5 px-3 py-1.5 bg-blue-50 hover:bg-blue-100 text-[#138FCB] rounded-lg text-xs font-bold border border-blue-200 transition-colors shadow-2xs cursor-pointer active:scale-98">
          ...
          <table class="w-full text-left text-xs">
            <thead class="bg-slate-50 text-slate-500 uppercase text-[10px] tracking-wider border-b border-slate-200">
              <tr>
                ...
                <th class="py-3 px-2 w-[24%] font-semibold text-center">Delivered from WH *</th>
                <th class="py-3 px-2 w-[24%] font-semibold text-center">Delivered from Office *</th>
                <th class="py-3 px-3 w-[15%] font-semibold text-right">Cargo Qty</th>
                <th class="py-3 px-2 w-[5%] font-semibold text-center">Action</th>
              </tr>
            </thead>
            <tbody id="gp-items-tbody" class="divide-y divide-slate-100 text-slate-700">
              ${isEdit && (gatepassToEdit.lines || []).length > 0
                ? gatepassToEdit.lines.map(l => renderRowHtml(l.variantId, l.warehouseQty, l.officeQty)).join('')
                : renderRowHtml(variants[0]?.id, '', '')}
            </tbody>
            <tfoot class="bg-slate-50/80 border-t-2 border-slate-200" data-purpose="staff-assignment-tfoot">
              <tr>
                <td class="p-3 align-top">
                  <div class="space-y-1">
                    <label class="text-xs font-bold text-slate-800 flex items-center gap-1.5">
                      ...
                      <span>Assign Station Staff:</span>
                    </label>
                    <p class="text-[10px] text-slate-500 leading-normal">
                      Select staff members below each facility to dispatch physical pick assignments.
                    </p>
                  </div>
                </td>
                <td class="p-2.5 align-top bg-blue-50/40 border-x border-slate-200">
                  <!-- Warehouse Staff directly under Delivered from WH -->
                  <div class="space-y-1.5">
                    <div class="flex items-center justify-between pb-1 border-b border-blue-200/60">
                      ...
                      <span class="text-[9px] text-blue-600 font-semibold">(wh-1)</span>
                    </div>
                    ${whStaff.map(staff => {
                      const isChecked = isEdit ? (gatepassToEdit.assignedStaffIds || []).includes(staff.id) : true;
                      return `
                        <label class="flex items-center gap-2 p-1.5 bg-white hover:bg-blue-50 rounded-lg border border-blue-200 cursor-pointer transition-all shadow-2xs">
                          <input type="checkbox" name="assignedStaff" value="${staff.id}" ${isChecked ? 'checked' : ''} class="w-3.5 h-3.5 rounded text-[#138FCB] focus:ring-0">
                ...
                <td class="p-2.5 align-top bg-amber-50/40 border-r border-slate-200">
                  <!-- Office Staff directly under Delivered from Office -->
                  <div class="space-y-1.5">
                    <div class="flex items-center justify-between pb-1 border-b border-amber-200/60">
                      ...
                      <span class="text-[9px] text-amber-600 font-semibold">(wh-2)</span>
                    </div>
                    ${officeStaff.map(staff => {
                      const isChecked = isEdit ? (gatepassToEdit.assignedStaffIds || []).includes(staff.id) : true;
                      return `
                        <label class="flex items-center gap-2 p-1.5 bg-white hover:bg-amber-50 rounded-lg border border-amber-200 cursor-pointer transition-all shadow-2xs">
                          <input type="checkbox" name="assignedStaff" value="${staff.id}" ${isChecked ? 'checked' : ''} class="w-3.5 h-3.5 rounded text-amber-600 focus:ring-0">
                          <span class="text-xs font-bold text-slate-800 truncate">${staff.fullName}</span>
                        </label>
                      `;
                    }).join('')}
                ...
                <td colspan="2" class="p-3 align-middle bg-slate-50/40">
                  <span class="text-[10px] text-slate-400 block italic leading-tight">Staff receive sirens &amp; notifications only if their hub has quantity &gt; 0</span>
                </td>
              </tr>
            </tfoot>
          </table>
        ...
        <div class="flex items-center justify-between pt-1">
          <button type="button" id="add-gp-row-btn-bottom" class="inline-flex items-center space-x-1.5 px-3.5 py-2 bg-slate-50 hover:bg-slate-100 text-slate-700 hover:text-[#138FCB] rounded-lg text-xs font-bold border border-dashed border-slate-300 hover:border-[#138FCB] transition-all cursor-pointer">
            <svg class="w-3.5 h-3.5 text-[#138FCB]" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
              <path d="M12 4v16m8-8H4" stroke-linecap="round" stroke-linejoin="round"></path>
            </svg>
      ...
      <!-- SECTION 3: Bottom Dual Columns (Notes vs Summary) -->
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
        <!-- Notes -->
        <div class="lg:col-span-7 bg-white p-5 rounded-xl border border-slate-200/80 shadow-xs space-y-2" data-purpose="terms-and-notes">
          <label class="text-xs font-semibold text-slate-700" for="gp-notes">Dispatch Note / Special Instructions</label>
          <textarea class="w-full text-xs rounded-lg border border-slate-300 focus:border-[#138FCB] focus:ring focus:ring-blue-100 text-slate-700 p-2.5 resize-none shadow-2xs" id="gp-notes" placeholder="Add custom message, transport notes, or loading precautions..." rows="3">${isEdit ? (gatepassToEdit.notes || '') : ''}</textarea>
        </div>
        ...
        <div class="lg:col-span-5 bg-white p-5 rounded-xl border border-slate-200/80 shadow-xs space-y-3" data-purpose="totals-summary-card">
          ...
          <div class="space-y-2 text-xs">
            <div class="flex justify-between text-slate-600">
              ...
              <span id="summary-wh-qty" class="font-medium text-slate-900">0 PCS</span>
            </div>
            <div class="flex justify-between text-slate-600">
              <span>From Office (wh-2)</span>
              <span id="summary-off-qty" class="font-medium text-slate-900">0 PCS</span>
            </div>
            <div class="flex justify-between text-slate-600">
          ...
          <div class="mt-4 pt-3 bg-blue-50/50 -mx-5 -mb-5 p-5 rounded-b-xl border-t border-blue-100 flex items-center justify-between">
            ...
            <div class="text-right">
              <span id="summary-total-qty" class="text-2xl font-extrabold text-slate-900 tracking-tight">0 PCS</span>
            </div>
          </div>
        </div>
      </div>
  ...
  const footerHtml = `
    ...
    <div class="flex items-center space-x-3 w-full sm:w-auto justify-end">
      <button id="gp-cancel-btn" type="button" class="px-4 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50 rounded-lg transition-colors border border-slate-300 cursor-pointer">
        Cancel
      </button>
      <button type="submit" form="create-gp-form" class="inline-flex items-center space-x-2 px-5 py-2 text-xs font-bold text-white bg-[#138FCB] hover:bg-[#0E78AC] rounded-lg shadow-sm hover:shadow transition-all active:scale-[0.98] cursor-pointer">
  ...
  openModal({
    ...
    onOpen: (modalEl) => {
      ...
      const addRowBtnBottom = modalEl.querySelector('#add-gp-row-btn-bottom');
      const addRowBtnTop = modalEl.querySelector('#add-gp-row-btn-top');
      ...
      const updateRowCalculations = (row) => {
        const varSelect = row.querySelector('.gp-item-var');
        ...
        const vId = varSelect.value;
        const selectedOption = varSelect.selectedOptions[0];
        const unit = selectedOption ? selectedOption.getAttribute('data-unit') : 'PCS';
        const wStock = inventoryService.getBalance('wh-1', vId);
        const oStock = inventoryService.getBalance('wh-2', vId);

        whIndicator.textContent = `WH Stock: ${wStock.toLocaleString()} ${unit}`;
        offIndicator.textContent = `Office Stock: ${oStock.toLocaleString()} ${unit}`;

        const rawW = whQtyInput.value.trim();
        const rawO = offQtyInput.value.trim();
        ...
        if (!rawW && !rawO) {
          ...
          totalDisplay.textContent = `${lineTotal.toLocaleString()} ${unit}`;
        }
      };
      ...
      const appendNewRow = (variantId = null, whQty = '', offQty = '') => {
        ...
        tempDiv.innerHTML = renderRowHtml(variantId, whQty, offQty);
      ...
      if (addRowBtnBottom) addRowBtnBottom.onclick = handleAddRow;
      if (addRowBtnTop) addRowBtnTop.onclick = handleAddRow;
